...
PORT = int(os.environ.get("GALLERY_PORT", "7998"))
...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/api/v2/images":
            logger.info(
                "Запрос к /api/v2/images",
                extra={
        ...
        elif parsed_path.path.startswith("/api/v2/image/"):
            try:
                ...
                logger.info(
                    "Извлечён индекс",
                    extra={
                        "request": f"GET /api/v2/image/{index}",
                        "client_ip": self.client_address[0],
                        "public_ip": public_ip,
                        "response_status": "200",
                        "index": index,
                    ...
                    with cache_lock:
                        if cache_key in scale_cache:
                            ...
                            logger.info(
                                ...
                                extra={
                                    "request": f"GET /api/v2/image/{index} scale={scale}",
                                    "client_ip": self.client_address[0],
                                    "public_ip": public_ip,
                                    "response_status": "200",
                                    "file": file_name,
