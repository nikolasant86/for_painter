...
PORT = int(os.environ.get("GALLERY_PORT", "7997"))
...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/api/v3/images":
            logger.info(
                "Запрос к /api/v3/images",
                extra={
                    "request": "GET /api/v3/images",
                    "client_ip": self.client_address[0],
                    "public_ip": public_ip,
                    "response_status": "200",
                },
        ...
        elif parsed_path.path.startswith("/api/v3/image/"):
            try:
                ...
                logger.info(
                    "Извлечён индекс",
                    extra={
                        "request": f"GET /api/v3/image/{index}",
                        "client_ip": self.client_address[0],
                        "public_ip": public_ip,
                        "response_status": "200",
                        "index": index,
                    ...
                    with cache_lock:
                        if cache_key in scale_cache:
                            ...
                            logger.info(
                                ...
                                extra={
                                    "request": f"GET /api/v3/image/{index} scale={scale}",
                                    "client_ip": self.client_address[0],
                                    "public_ip": public_ip,
                                    "response_status": "200",
                                    "file": file_name,
